...
public class AtualizaParaEmTransportePedidoSteps {

    private Response response;
    private Long idPedido;

    @Dado("que informo um pedido que ja foi cadastrado")
    public void queInformoUmPedidoQueJaFoiCadastrado() {
    ...
    public void queInformoUmPedidoQueJaEstaComOStatusEmTransporte() {
    ...
    public void queInformoUmPedidoQueNaoExiste() {
        this.idPedido = System.currentTimeMillis();
    }

    @Quando("atualizo esse pedido para EM TRANSPORTE")
    public void atualizoEssePedidoParaEmTransporte() {
        RestAssured.baseURI = "http://localhost:8081";
        this.response = given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .when()
                .put(URL_PEDIDOS_COM_ID.replace("{idPedido}", this.idPedido.toString()));
    }

    @Entao("recebo uma resposta que o pedido foi atualizado para EM TRANSPORTE com sucesso")
    public void receboUmaRespostaQueOPedidoFoiAtualizadoParaEmTransporteComSucesso() {
        this.response
                .prettyPeek()
                .then()
    ...
    public void receboUmaRespostaQueOPedidoNaoFoiAtualizadoParaEmTransporteComSucesso() {
        this.response
                .prettyPeek()
                .then()
